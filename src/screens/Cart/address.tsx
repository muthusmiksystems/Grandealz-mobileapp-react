...
const Address = ({ route }) => {
    ...
    useEffect(()=>{
        ...
        console.log(addresslist.length,"length",listing)
    },[addresslist])
                    ...
                    {(addresslist.length > 0) ?
                        (typeUser === "payment") ?
                            (addresslist).map((data, index) => (
                                <TouchableOpacity onPress={() => setSelection(index)} key={index} style={{ width: "92%", alignSelf: "center", borderRadius: 20, backgroundColor: COLORS.white, marginHorizontal: "2%", marginTop: "5%", marginBottom: "0%", borderColor: (selection == index) ? COLORS.element : COLORS.white, borderWidth: 1, }}>
                            ...
                            :
                            (addresslist).map((data, index) => (
                                <View key={index} style={{ width: "92%", alignSelf: "center", borderRadius: 20, backgroundColor: COLORS.white, marginHorizontal: "2%", marginTop: "5%", marginBottom: "0%", borderColor: (selection == index) ? COLORS.element : COLORS.white, borderWidth: 1, }}>
                                    <View style={{ flexDirection: "row", width: "90%", borderRadius: 10, padding: "1%" }}>
                                        <View style={{ flexDirection: "column", width: "90%", }}>
                                            <View style={{ flexDirection: "row", width: "100%" }}>
                                                <Text style={{ color: COLORS.textHeader, fontSize: RFValue(13), ...FONTS.lexendregular, marginLeft: "3%", marginVertical: "1%" }}>{data.name}</Text>
